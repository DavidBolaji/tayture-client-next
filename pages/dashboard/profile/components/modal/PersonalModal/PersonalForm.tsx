...
const PersonalForm: React.FC<PersonalInformationCardProp> = ({
    ...
    const {mutate, isPending} = useMutation({
      mutationFn: async (data: PersonalInformationCardProp & {path: string} & {othersText: string; others: string[]}) => {
    ...
    >
      {({
        ...
        <Form className={`w-full md:px-10 px-10 ${regularFont.className}`} onSubmit={handleSubmit}>
          ...
          <p className="text-ash_400 -mt-6 mb-5 w-full">Select all that applies to you</p>
          <div>
            <Checkbox.Group
              ...
              <Row>
                {['parent', 'teacher', 'school admin'].map((p, id) => (
                  <Col span={24} className="mb-5 ml-2" key={`${p}_${id}`}>
                    ...
                    >
                      <Checkbox value={p} className="font-[500] text-[1rem] capitalize" checked>
                        {p}
                      </Checkbox>
                    </ConfigProvider>
                  </Col>
